...
// ── WebXR mouse-delta injection ───────────────────────────────────────────────
// JS XR frame callbacks call webxr_inject_mouse_delta() to push head-rotation
// deltas directly into MyEventReceiver, bypassing the SDL DOM event pipeline.
// webxr_consume_mouse_delta() is called once per game frame from getMouseMovement().
namespace {
    std::atomic<int> s_xr_dx{0};
    std::atomic<int> s_xr_dy{0};
    // WebXR chat queue: JS queues a UTF-8 message; game loop consumes it.
    std::mutex s_chat_mutex;
    std::string s_pending_chat;
    bool s_has_pending_chat = false;

    // WebXR absolute orientation (set each XR frame from JS).
    // yaw_deg  > 0 = turned right of calibration pose.
    // pitch_deg > 0 = looking up from calibration pose.
    // When valid, game.cpp's updateCameraOrientation() uses these directly
    // instead of accumulating mouse deltas, eliminating all gyro drift.
    std::atomic<float> s_xr_abs_yaw{0.0f};
    std::atomic<float> s_xr_abs_pitch{0.0f};
    std::atomic<bool>  s_xr_abs_valid{false};

    // Camera matrices pushed by game.cpp each frame; read by JS via
    // webxr_get_camera_matrices() to render world-space video billboards
    // with the exact view/projection the engine uses.
    float s_cam_view[16] = {};
    float s_cam_proj[16] = {};
    std::atomic<bool> s_cam_valid{false};

    // VR dominant (right) controller position in head-local space (meters).
    // +X = right, +Y = up, -Z = forward (WebXR convention).
    std::atomic<float> s_xr_rhand_lx{0.0f};
    std::atomic<float> s_xr_rhand_ly{0.0f};
    std::atomic<float> s_xr_rhand_lz{0.0f};
    std::atomic<bool>  s_xr_rhand_valid{false};

    // When true, the local player body mesh is rendered even in first-person.
    std::atomic<bool>  s_xr_body_visible{false};
}

void webxr_consume_mouse_delta(int &dx, int &dy)
{
    dx = s_xr_dx.exchange(0, std::memory_order_relaxed);
    dy = s_xr_dy.exchange(0, std::memory_order_relaxed);
}

void webxr_consume_look(float &yaw, float &pitch, bool &valid)
{
    valid = s_xr_abs_valid.load(std::memory_order_relaxed);
    if (valid) {
        yaw   = s_xr_abs_yaw.load(std::memory_order_relaxed);
        pitch = s_xr_abs_pitch.load(std::memory_order_relaxed);
    }
}

void webxr_push_camera_matrices(const float* view16, const float* proj16)
{
    memcpy(s_cam_view, view16, 64);
    memcpy(s_cam_proj, proj16, 64);
    s_cam_valid.store(true, std::memory_order_relaxed);
}

bool webxr_consume_pending_chat(std::string &out)
{
    std::lock_guard<std::mutex> lock(s_chat_mutex);
    if (!s_has_pending_chat) return false;
    out = std::move(s_pending_chat);
    s_has_pending_chat = false;
    return true;
}

void webxr_consume_right_hand(float &lx, float &ly, float &lz, bool &valid)
{
    valid = s_xr_rhand_valid.load(std::memory_order_relaxed);
    if (valid) {
        lx = s_xr_rhand_lx.load(std::memory_order_relaxed);
        ly = s_xr_rhand_ly.load(std::memory_order_relaxed);
        lz = s_xr_rhand_lz.load(std::memory_order_relaxed);
    }
}

bool webxr_is_body_visible()
{
    return s_xr_body_visible.load(std::memory_order_relaxed);
}
// ─────────────────────────────────────────────────────────────────────────────
...
extern "C" {
    ...
    // WebXR: JS calls this from the XR frame callback to inject head-rotation
    // deltas directly into the engine's input system.
    EMSCRIPTEN_KEEPALIVE
    void webxr_inject_mouse_delta(int dx, int dy)
    {
        s_xr_dx.fetch_add(dx, std::memory_order_relaxed);
        s_xr_dy.fetch_add(dy, std::memory_order_relaxed);
    }

    // WebXR absolute orientation: JS calls this every XR frame with the
    // headset orientation relative to the calibration pose captured at VR
    // start. yaw_deg > 0 = turned right; pitch_deg > 0 = looking up.
    // game.cpp's updateCameraOrientation() reads these and sets cam_yaw /
    // cam_pitch directly, bypassing all delta accumulation so drift is zero.
    EMSCRIPTEN_KEEPALIVE
    void webxr_set_look(float yaw_deg, float pitch_deg)
    {
        s_xr_abs_yaw.store(yaw_deg,   std::memory_order_relaxed);
        s_xr_abs_pitch.store(pitch_deg, std::memory_order_relaxed);
        s_xr_abs_valid.store(true,    std::memory_order_relaxed);
    }

    // WebXR absolute orientation: JS calls this when the VR session ends to
    // hand control back to the normal mouse/keyboard input pipeline.
    EMSCRIPTEN_KEEPALIVE
    void webxr_clear_look(void)
    {
        s_xr_abs_valid.store(false, std::memory_order_relaxed);
    }

    // JS reads camera matrices pushed by game.cpp each frame.
    // viewOut/projOut must each point to a 16-float (64-byte) buffer.
    // Returns 1 if matrices are available, 0 if game hasn't started yet.
    EMSCRIPTEN_KEEPALIVE
    int webxr_get_camera_matrices(float* viewOut, float* projOut)
    {
        if (!s_cam_valid.load(std::memory_order_relaxed)) return 0;
        memcpy(viewOut, s_cam_view, 64);
        memcpy(projOut, s_cam_proj, 64);
        return 1;
    }

    // WebXR: JS calls this to queue a chat message that the game loop will
    // deliver via client->typeChatMessage(), bypassing the key system so it
    // works even when inventory or other formspecs are open.
    EMSCRIPTEN_KEEPALIVE
    void webxr_queue_chat_message(const char *msg)
    {
        if (!msg || !*msg) return;
        std::lock_guard<std::mutex> lock(s_chat_mutex);
        s_pending_chat = msg;
        s_has_pending_chat = true;
    }

    // WebXR: JS calls this every XR frame with the dominant (right) controller
    // position in head-local space (meters, +X right, +Y up, -Z forward).
    // camera.cpp uses these to position the wield mesh at the physical hand.
    EMSCRIPTEN_KEEPALIVE
    void webxr_set_right_hand(float lx, float ly, float lz)
    {
        s_xr_rhand_lx.store(lx, std::memory_order_relaxed);
        s_xr_rhand_ly.store(ly, std::memory_order_relaxed);
        s_xr_rhand_lz.store(lz, std::memory_order_relaxed);
        s_xr_rhand_valid.store(true, std::memory_order_relaxed);
    }

    // WebXR: JS calls this with 1 when an immersive session starts and 0 when
    // it ends. Enables body mesh rendering in first-person so the player can
    // look down and see their own body.
    EMSCRIPTEN_KEEPALIVE
    void webxr_set_body_visible(int visible)
    {
        s_xr_body_visible.store(visible != 0, std::memory_order_relaxed);
        if (!visible)
            s_xr_rhand_valid.store(false, std::memory_order_relaxed);
    }
