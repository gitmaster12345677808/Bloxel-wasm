...
public:
	...
	void startup(volatile std::sig_atomic_t *kill,
			InputHandler *input,
...
void Game::startup(volatile std::sig_atomic_t *kill,
		InputHandler *input,
		RenderingEngine *rendering_engine,
...
void Game::processQueues(f32 dtime)
{
	texture_src->processQueue();
	shader_src->processQueue();
}
...
void Game::processUserInput(f32 dtime)
{
	...
	// WebXR VR chat: JS queues a message via webxr_queue_chat_message();
	// deliver it directly so it works even when a formspec (inventory) is open.
	{
		std::string pending_chat;
		if (webxr_consume_pending_chat(pending_chat) && !pending_chat.empty())
			client->typeChatMessage(utf8_to_wide(pending_chat));
	}
#endif
...
{
#ifdef __EMSCRIPTEN__
	// ── WebXR absolute-orientation override ────────────────────────────────
	// While a WebXR immersive-vr session is active the JS side calls
	// webxr_set_look() every XR frame with the headset orientation expressed
	// as (yaw_deg, pitch_deg) RELATIVE TO a calibration quaternion captured
	// at VR-start time.  Because the values are recomputed from the raw
	// WebXR quaternion each frame (not accumulated via deltas), drift is
	// structurally impossible: looking back to the centre always returns the
	// camera to exactly where it was.
	//
	// Convention (matching the existing mouse-delta pipeline):
	//   yaw_deg  > 0  →  turned right of calibration pose
	//   pitch_deg > 0 →  looking up   of calibration pose
	{
		float xr_yaw = 0.0f, xr_pitch = 0.0f;
		bool  xr_valid = false;
		webxr_consume_look(xr_yaw, xr_pitch, xr_valid);

		// Persistent state across frames (reset when VR session ends).
		static float s_initial_yaw   = 0.0f;
		static float s_initial_pitch = 0.0f;
		static bool  s_initialized   = false;

		if (xr_valid) {
			// First frame of this VR session: snapshot the current cam
			// orientation so the world keeps its original facing direction.
			if (!s_initialized) {
				s_initial_yaw   = cam->camera_yaw;
				s_initial_pitch = cam->camera_pitch;
				s_initialized   = true;
			}
			// Apply absolute orientation.  Minetest convention:
			//   camera_yaw   increases → looking right  (same sign as xr_yaw)
			//   camera_pitch increases → looking down   (opposite sign to xr_pitch)
			cam->camera_yaw   = s_initial_yaw   + xr_yaw;
			cam->camera_pitch = s_initial_pitch - xr_pitch;
			cam->camera_pitch = rangelim(cam->camera_pitch, -90, 90);
			return;
		}
		// VR session ended → reset so the next session re-captures the
...
#ifdef __EMSCRIPTEN__
	// Push view + projection matrices to JS so the VideoScreen billboard
	// can be rendered at a fixed world-space position each frame.
	{
		auto* cam = this->camera->getCameraNode();
		const core::matrix4& view = cam->getViewMatrix();
		const core::matrix4& proj = cam->getProjectionMatrix();
		webxr_push_camera_matrices(view.pointer(), proj.pointer());
	}
#endif
