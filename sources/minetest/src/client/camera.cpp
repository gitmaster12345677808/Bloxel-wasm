...
#ifdef __EMSCRIPTEN__
	// VR: place the wield mesh at the physical controller position.
	// Still apply dig/place animation as an offset so arm swings are visible.
	{
		float rh_lx = 0.0f, rh_ly = 0.0f, rh_lz = 0.0f;
		bool  rh_valid = false;
		webxr_consume_right_hand(rh_lx, rh_ly, rh_lz, rh_valid);
		if (rh_valid) {
			// head-local meters → wield scene units (+X right, +Y up, -Z fwd)
			wield_position = v3f(rh_lx * 100.0f,
			                     rh_ly * 100.0f,
			                     65.0f - rh_lz * 80.0f);
			// Preserve dig/place swing so block interactions feel responsive
			if (m_digging_button != -1) {
				f32 digfrac = m_digging_anim;
				wield_position.X -= 50 * std::sin(std::pow(digfrac, 0.8f) * M_PI);
				wield_position.Y += 24 * std::sin(digfrac * 1.8f * M_PI);
				wield_position.Z += 25 * 0.5f;
