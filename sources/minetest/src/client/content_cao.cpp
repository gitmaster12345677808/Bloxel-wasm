...
#ifdef __EMSCRIPTEN__
				// In VR first-person body mode: hide Head (camera is inside
				// it) and both Arms (the wield mesh is the VR hand visual).
				// Body and legs stay visible so the player can look down.
				if (m_is_local_player && webxr_is_body_visible()) {
					static const char *hide[] = {"Head","Arm_Left","Arm_Right"};
					for (const char *name : hide) {
						auto *bone = m_animated_meshnode->getJointNode(name);
						if (bone) bone->setScale(v3f(0.0f, 0.0f, 0.0f));
					}
				}
#endif
			});
...
void GenericCAO::updateNodePos()
{
	...
	if (node) {
		...
		v3f pos = pos_translator.val_current -
				intToFloat(camera_offset, BS);
#ifdef __EMSCRIPTEN__
		// In VR first-person body mode shift the mesh down so the shoulders
		// are well below the camera (eye ~1.625 BS, shoulders ~1.25 BS —
		// only 0.375 BS gap by default → immediate clip on any downward look).
		// Camera node is in a separate hierarchy and is unaffected.
		if (m_is_local_player && webxr_is_body_visible())
			pos.Y -= BS * 0.5f;
#endif
...
#ifdef __EMSCRIPTEN__
	// In VR, keep the body visible in first-person so the player can look
	// down and see their legs/torso.
	const bool hidden = (m_client->getCamera()->getCameraMode() == CAMERA_MODE_FIRST)
	        && !webxr_is_body_visible();
#else
	const bool hidden = m_client->getCamera()->getCameraMode() == CAMERA_MODE_FIRST;
#endif
