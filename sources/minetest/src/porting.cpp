...
//// Emscripten / WebAssembly — OPFS-backed persistent storage
#elif defined(__EMSCRIPTEN__)

bool setSystemPaths()
{
	// Mount an OPFS backend at /persist so all writes there survive page reloads.
	// Safe to call here because the WASM module runs in a Web Worker, not the
	// main browser thread, so wasmfs_create_opfs_backend() cannot deadlock.
	backend_t opfs = wasmfs_create_opfs_backend();
	if (wasmfs_create_directory("/persist", 0777, opfs) != 0) {
		warningstream << "setSystemPaths: failed to create /persist OPFS directory"
			<< std::endl;
	}
...
int mt_snprintf(char *buf, const size_t buf_size, const char *fmt, ...)
{
	...
	//  arguments (eg. "%1$s"). We just forward the call to vsnprintf for sane?proxy=0&game=minetest_game&join=EF04C62C6FD2
