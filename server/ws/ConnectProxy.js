...
const PAYLOAD = {
    'total': { 'servers': DIRECT_PROXY.length, 'clients': 111 },
    'total_max': { 'server': DIRECT_PROXY.length, 'clients': DIRECT_PROXY.length * 6 },
    'list': DIRECT_PROXY.map(([vip, ip, port, na, des]) => {
        return {
            'address': vip,
            'ip': vip,
            'port': port,
...
export class ConnectProxy {
    ...
    handle_handshake(data) {
        ...
        if (host != 'mail.etherdeck.org' || port !=3333) {
